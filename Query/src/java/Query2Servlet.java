...
public class Query2Servlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            sqlStatement = "SELECT DISTINCT DRUG.TYPE "
                            + "FROM DRUG, DISEASE, PATIENT, CLINICAL_FACT "
                            + "WHERE DRUG.DR_ID = CLINICAL_FACT.DR_ID AND "
                            + "DISEASE.DS_ID = CLINICAL_FACT.DS_ID AND "
                            + "PATIENT.P_ID = CLINICAL_FACT.P_ID AND "
                            + "DISEASE.DESCRIPTION = 'tumor'";
